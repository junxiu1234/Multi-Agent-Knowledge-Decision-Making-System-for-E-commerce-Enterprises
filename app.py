...
#         messages = st.session_state.sessions[thread_id]
#         if messages:
#             first_msg = messages[0]["content"][:20] + "..." if len(messages[0]["content"]) > 20 else messages[0]["content"]
...
# messages = st.session_state.sessions[current_thread_id]
#
# # ===================== 页面标题与说明 =====================
# st.title("📊 星衡健康多Agent决策系统")
# st.caption(f"当前会话：{current_thread_id} | 支持客诉处理、营销合规审核、内部制度咨询、供应链查询 | 基于LangGraph + 多轮记忆")
# st.divider()
#
# # ===================== 聊天历史展示 =====================
# for msg in messages:
...
# st.caption("- 会话记忆基于内存存储（重启页面后丢失，如需持久化可联系开发升级）")
